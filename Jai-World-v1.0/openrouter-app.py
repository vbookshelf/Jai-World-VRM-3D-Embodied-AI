...
"y-bot" and the AI-controlled "x-bot") to a model on OpenRouter via
...
# ─── Model config ──────────────────────────────────────────────────────────
# Any OpenRouter model that supports tool/function calling works here.
# "provider" pins routing to a single upstream provider (no fallbacks) —
# set to None to let OpenRouter pick automatically.
SUPPORTED_MODELS = [
    {
...
# (see stream_agent_turn's handling of the "_image" key).
...
def build_look_around_composite():
    ...
    composite_b64_raw has no data-URL prefix, for embedding in an OpenAI-style
    image_url content part. If any direction hasn't been captured yet,
...
def build_tools_and_dispatch(live_state):
    ...
    def look_around(_params):
        ...
        return {
            ...
            # Leading underscore marks this as image payload: stream_agent_turn strips
...
def stream_agent_turn(user_message, live_state, status_line):
    ...
    Follows the same streaming shape as open-router-example.py's
    stream_response(): unlike Ollama, OpenAI-compatible streaming delivers
    tool call *arguments* as fragments spread across many chunks (keyed by
    index), not as one complete block, so they have to be accumulated
    incrementally before the JSON can be parsed. Text content deltas arrive
    plainly and are forwarded to the caller as they arrive; only once a
    given iteration's stream is exhausted do we know whether it ended in
    tool_calls (dispatch and loop again) or a plain final answer (done).
    """
    ...
    # Pin routing to a single upstream provider (no silent fallbacks) and
    # turn thinking/reasoning off, same as open-router-example.py.
    extra_body = {"reasoning": {"enabled": False}}
    if SELECTED_MODEL.get("provider"):
        extra_body["provider"] = {
            "order": [SELECTED_MODEL["provider"]],
            "allow_fallbacks": False,
        }

    for iteration in range(MAX_ITERATIONS):
        text_content = ""
        tool_calls_raw = {}  # index -> accumulated {"id", "name", "arguments"}
        usage_chunk = None

        # Snapshot under the lock rather than passing message_history itself:
        # this is a long-running network call, so we don't want to hold
        # STATE_LOCK for its duration, and we don't want a concurrent
        # /api/reset mutating the same list object while it's being iterated
        ...
        print(f"- Provider: {SELECTED_MODEL.get('provider') or 'auto'}")
        ...
        for chunk in stream:
            # OpenRouter's final SSE chunk of a streaming response carries a
            # populated `usage` object (tokens + cost in credits) instead of
            # a choices delta — see
            # https://openrouter.ai/docs/cookbook/administration/usage-accounting.
            # No extra request params are needed to get it; it's included
            ...
            if delta.tool_calls:
                for tc in delta.tool_calls:
                    ...
                    if i not in tool_calls_raw:
                        tool_calls_raw[i] = {"id": tc.id, "name": "", "arguments": ""}
                    if tc.function.name:
        ...
        if usage_chunk is not None:
            ...
            total_tokens = getattr(usage_chunk, "total_tokens", prompt_tokens + completion_tokens)
            ...
            print("[WARNING] No usage data returned for this call.\n")

        tool_calls_dicts = None
        if tool_calls_raw:
            # Persist as plain OpenAI-compatible dicts (not SDK objects) so
            # message_history can be sent straight back on the next request.
            tool_calls_dicts = [
                {
                    "id": tc["id"],
                    "type": "function",
            ...
            for call in tool_calls_dicts:
                ...
                with STATE_LOCK:
                    ...
                    if image_b64:
                        ...
                        # OpenAI/OpenRouter-style multimodal content: a list
                        # of parts on a user message, rather than Ollama's
                        # separate top-level "images" field.
                        image_msg = {
                            "role": "user",
                            "content": [
                                {
                                    "type": "text",
                                    "text": (
                                        "(Image attached from look_around: four labeled "
                                        "views — FRONT, RIGHT, REAR, LEFT — of x-bot's "
                                        "surroundings at the moment it looked. This "
                                        "reflects where x-bot was standing then, not "
...
def api_chat():
    ...
    # Computed once here (not inside stream_agent_turn) so it's available for the
    ...
    def generate():
        try:
            ...
            yield sse(
                "error",
                reply=(
                    "(x-bot's brain is offline — check that OPENROUTER_API_KEY is set "
                    f"and '{SELECTED_MODEL['model_id']}' is reachable via OpenRouter. "
                    f"Error: {e})"
                ),
                status_line=status_line,
            )
